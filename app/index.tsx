...
const API_BASE = process.env.EXPO_PUBLIC_API_BASE ?? 'http://127.0.0.1:8787';
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.footerCard}>
          <Text style={styles.footerTitle}>Produzione</Text>
          <Text style={styles.footerText}>
            {`Backend Render richiesto. Imposta EXPO_PUBLIC_API_BASE sul tuo URL Render. Ora: ${API_BASE}`}
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
...
async function readError(response: Response): Promise<string> {
  if (response.status === 401) {
    return 'Autenticazione fallita. Verifica email/password e token.';
  }
